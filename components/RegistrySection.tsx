        ...
        {gifts.map(gift => (
          <div key={gift.id} className="group relative bg-white rounded-[1.5rem] overflow-hidden shadow-sm transition-all duration-500 border border-[#F5F0EB]">
            <div className="h-48 overflow-hidden relative">
              <img src={gift.image} alt={gift.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              {gift.reserved && (
                <div className="absolute inset-0 bg-white/70 backdrop-blur-[1px] flex items-center justify-center">
                  <span className="px-4 py-1.5 bg-secondary text-white text-[9px] font-bold tracking-[0.2em] rounded-full">
                    DIHADIAHKAN
                  </span>
                </div>
              )}
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-primary font-bold text-xs mb-4 opacity-70">{gift.price}</p>
